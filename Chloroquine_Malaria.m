function [Parasites_at_end, percentCured] = Chloroquine_Malaria(DosingRegimen, doseRange,rangekP,rangeBurden, FirstDosing,OtherDosing,timepoints,kP_vector, P0_vector,FirstDoseRange, SecondDoseRange, patients, MIC, MissedDose,DisplayPlots)
...
for z = 1:doseRange %run simulations for ALL the doses (currently we have 10 dose conditions) 
    [~,~,~,~,~,~,~,~,~,~,Time,YCQCentral,~,AUCCQ,~] = Chloroquine_Main(DosingRegimen, FirstDoseRange(z),SecondDoseRange(z), MissedDose, DisplayPlots);
...
for patient =1:patients %iterate through all 100 patients
    ...
    tspan = 0:1.5:Time(end); %if the patient never drops below MIC, simulate ALL observation time
    ...
    tspan1 = 0:1.5:Time(droppoint,1); %index the Time vector get the TIME (in hrs) where the conc falls below 0 
...
percentCured = zeros(doseRange, rangekP); %index position 2 will eventually be filled in by kP value
