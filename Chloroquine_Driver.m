...
kP = linspace(1/25,1/110,patients)'; %hr-1, create a parasite clearance rate for each patient
...
for i =1:patients
    if CQconcBelow(1,i) ==0 %just looking at all 100 patients for the first dose condition (dose conditions 1/10)
    tspan = 0:.06:500; %if the patient never drops below MIC, simulate ALL observation time (500 hours)
    ...
    droppoint = CQconcBelow(1,i); %time point at which the patient's [CQ] drops below MIC
    tspan1 = 0:0.06:Time(droppoint,1); %index the Time vector get the TIME (in hrs) where the conc falls below 0 
    ...
    tspan2 = tspan1(end):.06:Time(end); %new tspan goes from end of first tspan to end of simulation (500 hrs)
